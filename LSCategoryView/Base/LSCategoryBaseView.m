...
@interface LSCategoryBaseView () <UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout>
...
@implementation LSCategoryBaseView
...
- (void)initializeData
{
    ...
    _selectedIndex = 0;
    _cellWidth = LSCategoryViewAutomaticDimension;
    ...
    _contentEdgeInsetLeft = LSCategoryViewAutomaticDimension;
    _contentEdgeInsetRight = LSCategoryViewAutomaticDimension;
    _lastContentViewContentOffset = CGPointZero;
}

- (LSCategoryCollectionView *)collectionView{
    if (!_collectionView) {
        UICollectionViewFlowLayout *layout = [[UICollectionViewFlowLayout alloc] init];
        layout.scrollDirection = UICollectionViewScrollDirectionHorizontal;
        LSCategoryCollectionView *collectionView = [[LSCategoryCollectionView alloc] initWithFrame:self.bounds collectionViewLayout:layout];
...
- (void)reloadCellAtIndex:(NSInteger)index {
    ...
    LSCategoryBaseCellModel *cellModel = self.dataSource[index];
    [self refreshCellModel:cellModel index:index];
    LSCategoryBaseCell *cell = (LSCategoryBaseCell *)[self.collectionView cellForItemAtIndexPath:[NSIndexPath indexPathForItem:index inSection:0]];
...
- (void)refreshState {
    ...
    for (int i = 0; i < self.dataSource.count; i++) {
        LSCategoryBaseCellModel *cellModel = self.dataSource[i];
        cellModel.index = i;
    ...
    if (self.averageCellSpacingEnabled && totalItemWidth < self.bounds.size.width) {
        ...
        if (self.contentEdgeInsetLeft == LSCategoryViewAutomaticDimension) {
            cellSpacingItemCount += 1;
        }else {
            totalCellSpacingWidth -= self.contentEdgeInsetLeft;
        }
        //如果内容右边距是Automatic，就加1
        if (self.contentEdgeInsetRight == LSCategoryViewAutomaticDimension) {
            cellSpacingItemCount += 1;
        }else {
        ...
        [self.dataSource enumerateObjectsUsingBlock:^(LSCategoryBaseCellModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
    ...
    [self.dataSource enumerateObjectsUsingBlock:^(LSCategoryBaseCellModel * cellModel, NSUInteger idx, BOOL * _Nonnull stop) {
    ...
    if (CGRectEqualToRect(self.contentScrollView.frame, CGRectZero) && self.contentScrollView.superview != nil) {
        //某些情况、系统会出现LSCategoryView先布局，contentScrollView后布局。就会导致下面指定defaultSelectedIndex失效，所以发现frame为zero时，强行触发布局。
...
- (BOOL)_selectCellAtIndex:(NSInteger)targetIndex handleContentScrollView:(BOOL)handleContentScrollView isClicked:(BOOL)isClicked{
    if (targetIndex >= self.dataSource.count || self.selectedIndex == targetIndex) {
        return NO;
    }
    
    LSCategoryBaseCellModel *lastCellModel = self.dataSource[self.selectedIndex];
    LSCategoryBaseCellModel *selectedCellModel = self.dataSource[targetIndex];
    [self refreshSelectedCellModel:selectedCellModel unselectedCellModel:lastCellModel];
    
    LSCategoryBaseCell *lastCell = (LSCategoryBaseCell *)[self.collectionView cellForItemAtIndexPath:[NSIndexPath indexPathForItem:self.selectedIndex inSection:0]];
    [lastCell reloadData:lastCellModel];
    
    LSCategoryBaseCell *selectedCell = (LSCategoryBaseCell *)[self.collectionView cellForItemAtIndexPath:[NSIndexPath indexPathForItem:targetIndex inSection:0]];
...
- (void)contentOffsetOfContentScrollViewDidChanged:(CGPoint)contentOffset {
    ...
    if (remainderRatio == 0) {
        ...
        if (self.cellWidthZoomEnabled && self.cellWidthZoomScrollGradientEnabled) {
            ...
            leftCellModel.cellWidthZoomScale = [LSCategoryFactory interpolationFrom:self.cellWidthZoomScale to:1.0 percent:remainderRatio];
            rightCellModel.cellWidthZoomScale = [LSCategoryFactory interpolationFrom:1.0 to:self.cellWidthZoomScale percent:remainderRatio];
            [self.collectionView.collectionViewLayout invalidateLayout];
        }
    }
}
...
- (Class)preferredCellClass {
    return LSCategoryBaseCell.class;
}
...
- (void)collectionView:(UICollectionView *)collectionView willDisplayCell:(UICollectionViewCell *)cell forItemAtIndexPath:(NSIndexPath *)indexPath {
    LSCategoryBaseCell *categoryCell = (LSCategoryBaseCell *)cell;
...
{
    LSCategoryBaseCellModel *cellModel = self.dataSource[indexPath.item];
